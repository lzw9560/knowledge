...
action_type: 链接维护
trigger: 报告入库后，扫描正文 6 位代码并建 [[stocks/代码]] 链接
...
- **类型**：链接维护
- **触发条件**：报告（`reports/` 或 `daily/` 下）新建/更新后触发
- **目标实体**：报告文件 → `stocks/` 实体（正反向链接）
- **审计**：true
...
- 触发自：[[logic/报告图谱关联]]（REPORT-LINK-001）
- 作用于：[[reports/]]、`daily/` 报告 → [[stocks/]]
- 执行记录：[[reviews/]]
