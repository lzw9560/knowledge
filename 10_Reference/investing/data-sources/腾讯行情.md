...
- 🏗️ H_Reference/tech-learning/architecture/分层架构|分层架构]] — 腾讯行情为底座层（L1），无降级，上层依赖此层
- 🔧 H_Reference/tech-learning/concepts/缓存策略|缓存策略]] — 高频行情入缓存降低请求
- 🔧 H_Reference/tech-learning/concepts/数据契约|数据契约]] — 行情返回由 Pydantic 契约层统一形状
