...
## 与其他项目的关系
- 依赖 [[10_Reference/projects/active/知识图谱|knowledge-graph]]（被索引的语料源）
- 支撑 [[10_Reference/projects/active/知识图谱Bot|knowledge-graph-]]（飞书 Bot 的自然语言→实体路由）
- 复用 [[10_Reference/projects/active/Vibe-Research|Vibe-]] 的 LLM/embedding 基础设施
- 与现有 `` 链接互补：`` 是人工显式边，语义搜索是算法隐式边

## 方法论意义
本项目的深层意义是"知识图谱第四构件——逻辑规则"的自动化：四构件方法论（[[10_Reference/meta/四构件本体方法论|四构件本体方法论]]）中，实体/关系/动作可人工建，但跨域"同构关系"（如"em_get 限流 ≅ 涨跌停板"）靠人工发现成本高——语义搜索可批量发现跨领域同构，作为人工补链的候选源。

## 待解决问题
- embedding 模型选型（中文投研语料的召回质量）
- 增量索引（vault 更新时只 re-embed 改动文件，见 [[10_Reference/projects/backlog/增量同步自动化|增量同步自动化]]）
- 召回结果的可解释性（为什么这个实体相关）

## 相关链接
- [[10_Reference/reading/MOC]]
- [[10_Reference/projects/active/知识图谱]]
- [[10_Reference/projects/active/知识图谱Bot]]
