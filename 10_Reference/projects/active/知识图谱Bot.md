...
## 项目概述
将 Vibe-Research 投研能力 + Obsidian 知识图谱通过飞书 Bot 接入日常沟通流。目标是"在飞书里自然语言问投研问题，Bot 查知识图谱 + 实时数据回答"。复用 [[10_Reference/projects/active/Vibe-Research|Vibe-]] 后端 API + [[10_Reference/projects/active/知识图谱|knowledge-graph]] 作为知识源，飞书侧只做消息桥接。

## 技术栈
- 飞书侧：飞书开放平台 Bot（长连接 WebSocket / Webhook）
- 后端：[[10_Reference/tech-learning/frameworks/fastapi|FastAPI]]（复用 Vibe-Research API）
- 知识源：[[10_Reference/projects/active/知识图谱|Obsidian]]（投研实体/战法/数据源/spec）
- 实时数据：[[10_Reference/investing/data-sources/东财 push2|东财]] / [[10_Reference/investing/data-sources/腾讯行情|腾讯行情]]
...
## 与其他项目的关系
- 依赖 [[10_Reference/projects/active/Vibe-Research|Vibe-]]（后端 API + 数据源层）
- 依赖 [[10_Reference/projects/active/知识图谱|knowledge-graph]]（投研语义层）
- 复用 Vibe-Research 的 [[10_Reference/investing/specs/archive/m0-foundation/S007-契约层|契约层]]（Pydantic 模型可直接序列化为飞书卡片 JSON）
- 对照 [[10_Reference/projects/backlog/微信Bot|wechat-]]——飞书 Bot 是企业侧入口，微信 Bot 是个人社交侧入口
