...
- 与 H_Reference/tech-learning/architecture/事件驱动架构|事件驱动]] 的关系：事件驱动是"通信模式"，事件溯源是"存储模式"——可叠加但不同概念
...
- 参考 H_Reference/tech-learning/architecture/事件驱动架构|事件驱动架构]] 的基础设施（事件总线）
