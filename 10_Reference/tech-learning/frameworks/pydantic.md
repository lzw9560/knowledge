...
- **契约层核心**：[[10_Reference/investing/specs/S007-契约层|S007 契约层]] 用 Pydantic 定义所有 API 请求/响应模型
- **数据层**：[[10_Reference/investing/specs/S008-后端数据层迁移|S008 后端数据层迁移]] 把 dataclass 迁到 Pydantic，统一验证 + 序列化
- **前后端类型同步**：[[10_Reference/investing/specs/S009-前后端类型同步|S009]] 从 Pydantic 模型生成 TypeScript 类型，供 [[frameworks/react|React]] 前端使用
