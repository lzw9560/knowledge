...
## 类别
- Unix 定时任务调度器
- 与 H_Reference/tech-learning/tools/GitHub-Actions|GitHub Actions]] 的 `schedule` 互为同构：本地用 cron，云端用 Actions schedule
...
- 与事件驱动 H_Reference/tech-learning/architecture/事件驱动架构|事件驱动]] 的关系：cron 是"时间触发器"，事件驱动是"事件触发器"，二者在 S011 中合并为统一调度层
