...
- [[A股交易成本结构]]（logic 候选）——成本三组件（5元最低佣金+印花0.05%+滑点0.70%往返+T+1）+ 单笔曲线 floor@2万 + 周期分布（超短吃60%+/长线<3%）
...
- [[A股异常交易监管]]（logic 候选）——证券法 55/56/192 + 虚假申报阈值 + 连板特停偏离值 + 2025 程序化新规
- [[打板形态§44verdict图]]（metric 候选）——recorder.db 83 行打板各形态 lift/p/status 核实图
...
- [[红利低波复合因子]]（metric 候选）——A 股长线最硬，三重定论（学术+卖方+指数），年化9-12%回撤<14%熊市逆势+42%
- [[动量因子_A股]]（metric 候选）——A 股动量长期无效，别套美股；反转 IC=0.068/IR=0.447 但 2020 后衰减
- [[zlotus-ash-mcp]]（data_source 候选）——baostock+akshare MCP，自带价值因子评分+低估筛选+再平衡，最对路 fork
- [[qlib]]（data_source 候选）——Alpha158 纯量价无价值因子，纠偏「qlib wrap 价值」误传

## 待办（promote 候选，定论后迁移）

- 用户「都做」启动后，**指数复制臂**（akshare 取 930955 成分+半年调样）跑通 + 红利低波结论定型 → promote [[红利低波复合因子]] 到 `10_Reference/investing/metrics/`
- **ash-mcp 臂**真 clone+wire+跑通 → promote [[zlotus-ash-mcp]] 到 `10_Reference/investing/data-sources/`
- [[A股交易成本结构]] 被多轮引用定型 → promote 到 `10_Reference/investing/logic/`
- [[动量因子_A股]]/[[qlib]] 作为「纠偏/避坑」知识定型后 promote
